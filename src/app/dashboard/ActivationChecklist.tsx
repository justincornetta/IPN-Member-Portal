  ...
  return (
    <section className="rounded-xl border border-ipn/15 bg-white p-4 shadow-sm sm:p-5" aria-labelledby="activation-heading">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 id="activation-heading" className="text-xs font-semibold uppercase tracking-[0.14em] text-ipn">Getting Started</h2>
      ...
      <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[#EDE5F7]" aria-hidden="true"><div className="h-full rounded-full bg-ipn transition-[width]" style={{ width: `${(summary.completedCount / items.length) * 100}%` }} /></div>
      <ol className="mt-3 overflow-hidden rounded-xl border border-[#E8E0F2] bg-white">
        {items.map((item, index) => {
          ...
          const classes = `flex min-h-16 items-center gap-3 px-3 py-3 text-left transition ${item.completed ? "bg-emerald-50/40" : "bg-white hover:bg-[#FAF7FF]"}`
          if (item.id === "whatsapp") return <li key={item.id} className="border-b border-[#EEE9F3]"><div className={`${classes} flex-col items-stretch`}><Link href={item.href!} className="flex w-full items-start gap-3">{lead}</Link>{!item.completed && <button type="button" onClick={confirmWhatsAppMembership} disabled={isConfirmingWhatsApp} className="ml-11 inline-flex min-h-8 items-center self-start rounded-md border border-ipn/20 px-2.5 py-1 text-[11px] font-semibold text-ipn hover:bg-ipn-light">{isConfirmingWhatsApp ? "Saving…" : "I’m already in"}</button>}</div>{whatsappConfirmationError && <p className="border-t border-[#EEE9F3] px-3 py-2 text-xs text-red-700" role="alert">We couldn’t save that confirmation. Try again.</p>}</li>
          if (item.id === "tour") return <li key={item.id} className="border-b border-[#EEE9F3]"><button type="button" onClick={startOrResume} className={`${classes} w-full`}>{lead}<ArrowIcon /></button></li>
          if (item.id === "participate") return <li key={item.id} className="relative"><button type="button" onClick={() => setParticipationOpen((open) => !open)} aria-expanded={participationOpen} className={`${classes} w-full`}>{lead}<ArrowIcon /></button>{participationOpen && <div className="grid gap-1 border-t border-[#EEE9F3] bg-[#FAF7FF] p-2"><Link href="/dashboard/events" className="rounded-md bg-white px-3 py-2 text-xs font-semibold text-ipn hover:bg-ipn-light">RSVP for an event</Link><Link href="/dashboard/directory" className="rounded-md bg-white px-3 py-2 text-xs font-semibold text-ipn hover:bg-ipn-light">Connect with a member</Link></div>}</li>
          return <li key={item.id} className="border-b border-[#EEE9F3]"><Link href={item.href!} className={classes}>{lead}<ArrowIcon /></Link></li>
        })}
      </ol>
    </section>
  )
