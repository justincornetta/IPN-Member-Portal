...
function ResourceMedia({ resource }: { resource: ResourceRecord }) {
  ...
  const wrapperClass = showContain
    ? "aspect-video overflow-hidden rounded-t-lg border-b border-zinc-200 bg-white sm:rounded-lg sm:border"
    : "aspect-video overflow-hidden rounded-t-lg bg-zinc-100 sm:rounded-lg"
...
function ResourceCard({ resource }: { resource: ResourceRecord }) {
  ...
  const body = (
    <article
      className={`flex h-full flex-col overflow-hidden rounded-lg border bg-white shadow-sm transition sm:p-4 ${
        isBlog
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-3 sm:mt-4 sm:p-0">
        <span
          className={`hidden w-fit rounded-md px-2 py-1 text-[11px] font-medium sm:inline-block ${
        ...
        <h3 className="text-base font-semibold leading-snug text-zinc-900 sm:mt-3">
          {resource.title}
        </h3>
        ...
        {resource.description && (
          <p className={`mt-1.5 ${descriptionClamp} text-sm leading-5 text-zinc-500 sm:mt-2 sm:leading-6`}>
            {resource.description}
          </p>
        )}

        {resource.benefit_note && (
          <div className="mt-4 hidden rounded-lg border border-ipn/20 bg-ipn/5 px-3 py-2 sm:block">
...
function BlogIdeaCard({ compact = false }: { compact?: boolean }) {
  return (
    <article className={`flex h-full flex-col rounded-lg border border-dashed border-ipn/30 bg-ipn/5 ${compact ? "p-3" : "min-h-[28rem] p-5"}`}>
      <p className="text-sm font-medium text-ipn">Have an article idea?</p>
      <h3 className="mt-1 text-base font-semibold text-zinc-900">
        Submit a blog pitch
      </h3>
      <p className={`mt-2 flex-1 text-sm text-zinc-600 ${compact ? "line-clamp-2 leading-5" : "leading-6"}`}>
...
export default function ResourcesHub({ resources }: Props) {
  ...
  const [visibleResourceCount, setVisibleResourceCount] = useState(3)
  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-6">
          ...
          <div className="grid grid-cols-1 gap-2 text-center sm:grid-cols-3">
            {tabs.map((tab) => (
              <button
                ...
                className="min-h-16 rounded-lg border border-white bg-white px-3 py-3 text-left shadow-sm transition hover:border-ipn/20"
              >
      ...
      <div className="grid grid-cols-3 gap-1 rounded-lg border border-zinc-200 bg-white p-1 shadow-sm">
        {tabs.map((tab) => (
          <button
            ...
            type="button"
            onClick={() => setResourceTab(tab.id)}
            className={`min-h-11 rounded-md px-1.5 py-2 text-xs font-medium transition min-[380px]:text-sm ${
              activeTab === tab.id
          ...
          {benefits.length ? (
            <div className="flex flex-col gap-3 sm:block">
              <div className="grid gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {visibleBenefits.map((resource) => (
                  <ResourceCard key={resource.id} resource={resource} />
                ))}
              </div>
              {isCompactMobile && visibleResourceCount < benefits.length && (
                <button
                  type="button"
      ...
      {activeTab === "blog_post" && (
        <section className="flex flex-col gap-4">
          <div className="grid gap-3 sm:gap-4 lg:grid-cols-[1fr_20rem] lg:items-end">
            <div className="hidden sm:block">
          ...
          {blogs.length ? (
            <div className="flex flex-col gap-3 sm:block">
              <div className="sm:hidden">
                <BlogIdeaCard compact />
              </div>
              <div className="grid gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <div className="hidden sm:block">
                  <BlogIdeaCard />
                </div>
                {visibleBlogs.map((resource) => (
                  <ResourceCard key={resource.id} resource={resource} />
                ))}
              </div>
              {isCompactMobile && visibleResourceCount < filteredBlogs.length && (
                <button
                  type="button"
                  onClick={() => setVisibleResourceCount((count) => count + 3)}
                  className="min-h-11 rounded-lg border border-zinc-200 bg-white px-4 py-2 text-sm font-medium text-zinc-700 shadow-sm"
      ...
      {activeTab === "partner" && (
        <section className="flex flex-col gap-4">
          ...
          {partners.length ? (
            <div className="flex flex-col gap-3 sm:block">
              <div className="grid gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {visiblePartners.map((resource) => (
                  <ResourceCard key={resource.id} resource={resource} />
                ))}
              </div>
              {isCompactMobile && visibleResourceCount < partners.length && (
                <button
                  type="button"
                  onClick={() => setVisibleResourceCount((count) => count + 3)}
                  className="min-h-11 rounded-lg border border-zinc-200 bg-white px-4 py-2 text-sm font-medium text-zinc-700 shadow-sm"
