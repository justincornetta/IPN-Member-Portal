...
export default function PastConferenceCard({
  conference,
  preview = false,
  compact = false,
}: {
  conference: PastConferenceRecord
  preview?: boolean
  compact?: boolean
}) {
  ...
  return (
    <article className={`flex h-full min-w-0 flex-col rounded-lg border border-zinc-200 bg-white p-3 shadow-sm ${compact ? "" : "sm:p-4"}`}>
      <ConferenceCover
      ...
      <div className={`mt-3 flex flex-1 flex-col ${compact ? "" : "sm:mt-4"}`}>
      ...
      <div className="mt-4 pt-4">
        {conference.drive_folder_url && preview ? (
          <span className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm font-medium text-zinc-700 sm:min-h-0">
            View & share photos
            <ExternalLinkIcon />
          </span>
