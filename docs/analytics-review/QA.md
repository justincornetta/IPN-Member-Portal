...
- Applied only the additive Community vocabulary expansion to production Supabase through the authorized connector. Local migration: `supabase/migrations/20260918164335_analytics_community_event_labels.sql`. Remote migration versions may differ; inspect history before replaying. Verified the expanded check constraint, RLS still enabled, and no anon/authenticated table grants. No account, RSVP, or saved label rows were changed. UI changes are local and not yet pushed to PR #63.
