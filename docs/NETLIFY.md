...
|---|---|---|---|
| `NEXT_PUBLIC_SUPABASE_URL` | Production Supabase project URL | Staging Supabase project URL | Public anon client config. Deploy Previews should not point at production when testing seeded/demo data. |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Production Supabase anon key | Staging Supabase anon key | Public anon client config. Use the key from the same project as the preview URL. |
...
For one-off recording or QA data, keep the service-role key local and use a staging Supabase project. See [`docs/MEDIA_RECORDING_PREVIEW.md`](MEDIA_RECORDING_PREVIEW.md).
